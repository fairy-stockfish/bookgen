...
Engine::Engine(std::optional<std::string> path) :
    ...
    options.add("MoveScoreRange", Option(100, 0, 1000));
    options.add("AbsScoreRange", Option(100, 0, 1000));
    options.add("AbsScoreBias", Option(0, -1000, 1000));
    options.add("DepthFactor", Option(100, 0, 200));
    options.add("AbsMoveScore", Option(false));
    options.add("TrimFEN", Option(true));
    options.add("EPDPath", Option("book.epd"));
...
void Engine::multipv_gen(Search::LimitsType limits, Depth depth, std::set<std::string>& fens, Value range) {
    limits.startTime = now();

    // Create a temporary position for search to ensure clean state
    StateListPtr searchStates(new std::deque<StateInfo>(1));
    Position searchPos;
    searchPos.set(pos.fen(), options["UCI_Chess960"], &searchStates->back());

    threads.start_thinking(options, searchPos, searchStates, limits);
    wait_for_search_finished();

    std::vector<Move> good_moves;
    const auto& rootMoves = threads.main_thread()->worker->get_rootMoves();
    size_t PVIdx = threads.main_thread()->worker->get_pvIdx();
    ...
    for (Move m : good_moves) {
        ...
        if (depth <= ONE_PLY) {
            ...
            multipv_gen(limits, depth - ONE_PLY, fens, range * int(options["DepthFactor"]) / 100);
        }
        pos.undo_move(m);
    }
}

std::uint64_t Engine::perft_gen(Depth depth, std::set<std::string>& fens) {
    StateInfo st;
    ...
    for (const auto& m : MoveList<LEGAL>(pos)) {
        pos.do_move(m, st);
        nodes += perft_gen(depth - ONE_PLY, fens);
        pos.undo_move(m);
    }
    return nodes;
}
...
void Engine::generate(std::istringstream& is, std::set<std::string>& fens) {
    ...
    if (limits.perft)
        perft_gen(limits.perft * ONE_PLY, fens);
    else
        multipv_gen(limits, depth * ONE_PLY, fens, int(options["MoveScoreRange"]) * PawnValueEg / 100);
}

void Engine::filter(std::istringstream& is, std::set<std::string>& fens) {
    Search::LimitsType limits;
    std::string token;
    ...
    StateListPtr states;
    Position pos;
    std::set<std::string> filtered_fens;
    ...
    for (const auto& fen : fens)
    {
        ...
        const Search::RootMoves& rootMoves = threads.main_thread()->worker->get_rootMoves();
        size_t PVIdx = threads.main_thread()->worker->get_pvIdx();
        size_t multiPV = std::min((size_t)options["MultiPV"], rootMoves.size());
