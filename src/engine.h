...
class Engine {
   public:
    ...
    std::uint64_t perft_gen(Depth depth, std::set<std::string>& fens);
    void multipv_gen(Search::LimitsType limits, Depth depth, std::set<std::string>& fens, Value range);
    void filter(std::istringstream& is, std::set<std::string>& fens);
    void generate(std::istringstream& is, std::set<std::string>& fens);
