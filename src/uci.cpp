...
void UCIEngine::generate(std::istringstream& is) {
    engine.generate(is, fens);
}

void UCIEngine::filter(std::istringstream& is) {
    engine.filter(is, fens);
}

void UCIEngine::print() {
    for (const auto& fen : fens)
        sync_cout << fen << sync_endl;
}

void UCIEngine::save() {
    std::ofstream file(engine.get_options()["EPDPath"]);
    for (const auto& fen : fens)
        file << fen << std::endl;
}
